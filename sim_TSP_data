...
simu.multi <- function(Gnull, Gsig, Ntrain, Ntest, SDgenemu=5, DFgenesd=1, Clow=2, Cup=Inf, MuShift=3, SDshift=0.5,seed=15213){
  library(truncnorm)
  ### input
  ...
  for(i in 1:Gsig){
    while(length(unique(direction[i,]))==1){  # not the sample direction
      direction[i,]=sample(c(-1,1),size=K,replace=T)
    }
